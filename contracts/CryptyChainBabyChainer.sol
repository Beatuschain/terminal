...
pragma solidity ^0.8.0;
contract CryptyChainBabyChainer is ERC721WithPermit, Ownable {
    using Strings for uint256;
    ...
    /* public cost and mint limit */
    uint256 public cost = 1 ether / 2;
    ...
    /* wldiscount 80% of public price*/
    uint256 public wldiscount = 80;
    
    /* percent */
    address private immutable dev;
    uint256 private immutable devfee = 5;
    ...
    constructor(
		...
		uint _maxSupply
		) ERC721("CryptyChainBabyChainer", "CCBC") ERC721WithPermit(_DOMAIN_SEPARATOR) {
	...
	require(_mintAmount <= mintLimit, "exceed mint limit per TX");
	require(supply + _mintAmount <= maxSupply, "max NFT limit exceeded");

	uint amount;
	uint ownerMintedCount = addressMintedBalance[msg.sender];
	if ((whitelist[msg.sender] == true) && (ownerMintedCount + _mintAmount <= perAddressLimit)) {
	    amount = (cost * _mintAmount * wldiscount) / 100;
	    addressMintedBalance[msg.sender] += _mintAmount;
	} else {
	    amount = cost * _mintAmount;
	}
	require(msg.value >= amount, "insufficient funds");

	/* dev tax */
	uint dev_tax = amount * devfee / 100;	    
	payable(dev).transfer(dev_tax);
    ...
    function setWhitelist(address[] memory recipients) external onlyOwner {
    ...
    function setCost(uint _cost, uint _wldiscount) public onlyOwner {
	cost = _cost;
    ...
    function setPerAddressLimit(uint _perAddressLimit) public onlyOwner {
	perAddressLimit = _perAddressLimit;
    }
